...
const CLOSED_ERROR = "Zigma Points is closed until the next term begins."

export async function POST(req: NextRequest): Promise<NextResponse> {
  // Between terms nobody gets an account. This needs its own gate: the middleware matcher
  // excludes /api/auth/* (NextAuth's handlers must stay unrestricted), and this is a plain
  // route handler that writes with db.user.create — it never reaches the signIn callback
  // in auth.ts that blocks the login half.
  if ((await downtimeState()).down) {
    return NextResponse.json({ error: CLOSED_ERROR }, { status: 503 })
  }
