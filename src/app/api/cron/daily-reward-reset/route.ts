...
export async function GET(req: Request) {
  ...
  // NOT gated on downtime, deliberately — an early return here loses real money. This
  // route pays previousDayKey(), so the 00:00 ET run on the morning after a term ends is
  // already in downtime and is the ONLY run that will ever pay the term's final day of
  // leaderboard prizes. Skip it and the last day's top-3 across all six games are never
  // paid, unrecoverably: no later run looks further back than yesterday.
  //
  // Letting it through costs nothing on the remaining days of a break. Every payout is
  // keyed to a day with no runs on it, so all six find no winners and write nothing. Only
  // the broadcast is suppressed below — that is the part users would actually see.
  ...
  // Silent during downtime: "games have reset, come back and play" pointing at a site
  // that redirects every path to /downtime is the one thing users must not receive.
  if (!down) after(() => notifyDailyRewardReady())
