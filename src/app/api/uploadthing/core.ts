...
/**
 * The gate both uploaders share: a live session AND an open term.
 *
 * The downtime half matters because this route is excluded from the middleware matcher
 * (Uploadthing's callback carries an HMAC signature, never a cookie — see middleware.ts),
 * so nothing upstream stops a stale session from spending storage while the app is shut.
 * The avatar uploader also writes to the user row on completion, which is a state change
 * during a period defined by having none.
 */
async function requireOpenSession() {
  // Auth first: an anonymous caller should learn "Unauthorized", not the app's schedule.
  const session = await auth()
  if (!session?.user?.id) throw new UploadThingError("Unauthorized")
  if ((await downtimeState()).down) {
    throw new UploadThingError("Zigma Points is closed until the next term begins.")
  }
