...
export default async function PostDetailPage({
  ...
  const exists = await db.post.findUnique({ where: { id }, select: { id: true, termId: true } })
  if (!exists) notFound()

  // A past term's thread is read-only. Derived here, on the server, off the existence
  // check that already runs — no extra client round-trip, and it matches the rule
  // reply.createReply enforces (the composer is an affordance, that guard is the control).
  const threadClosed = !(await isCurrentTerm(exists.termId))
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="mt-4">
        <ThreadSection postId={id} closed={threadClosed} />
      </div>
    </main>
  )
}
