...
export default async function TaskDetailPage({
  ...
  const task = await db.task.findUnique({
    ...
    select: {
      ...
      winningChoice: true,
      betSettledAt: true,
      termId: true, // drives the read-only thread on a past term's task
      mediaUrl: true,
      images: true,
      createdAt: true,
  ...
  // Same read-only rule as /post/[id]: a past term's thread takes no new replies.
  const threadClosed = !(await isCurrentTerm(task.termId))

  return (
    <main className="max-w-2xl mx-auto px-4 py-6">
      {/* Task card (read-only — no vote buttons on tasks) */}
      <TaskCard
          termClosed={threadClosed}
        id={task.id}
        title={task.title}
      ...
      <div className="mt-4">
        <TaskThreadSection taskId={task.id} closed={threadClosed} />
      </div>
    </main>
  )
}
