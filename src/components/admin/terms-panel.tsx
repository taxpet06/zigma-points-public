...
function TermRow({
  ...
  const invalidate = () => {
    ...
    void queryClient.invalidateQueries(trpc.term.poopHolders.queryFilter())
    void queryClient.invalidateQueries(trpc.term.poopers.queryFilter())
  ...
  return (
    <li className="rounded-lg border p-4">
      ...
      <PooperEditor termId={term.id} termName={term.name} users={users} onChange={invalidate} />
      {error && (
...
// A term's Zigma Poopers. The mirror of the Maxxer picker directly above it, except a
// term has any number of them, so this is add-and-remove rather than one <select>.
//
// Chips + an "add" <select> rather than a multi-select listbox: a native
// <select multiple> hides the current selection behind scrolling, needs ctrl-click to
// deselect, and is close to unusable on a phone — while the whole point of this control
// is seeing at a glance who is currently disqualified.
//
// Writes the WHOLE list every time (term.setPoopers replaces it in one transaction), so
// a double-click cannot half-apply and there is no add/remove ordering to get wrong.
function PooperEditor({
  termId,
  termName,
  users,
  onChange,
  ...
  const trpc = useTRPC()
  const { data: rows } = useQuery(trpc.term.poopers.queryOptions({ termId }))
  const current = rows?.map((r) => r.userId) ?? []

  const setPoopers = useMutation(
    trpc.term.setPoopers.mutationOptions({
      onSuccess: (res) => {
        toast.success(
          res.clearedWinner
            ? "Disqualified — they were this term's Maxxer, so the winner and crown were cleared."
            : "Zigma Poopers updated",
        )
        onChange()
      },
      onError: (e) => toast.error(e.message || "Couldn't update the Poopers."),
    }),
  )

  const save = (userIds: string[]) => setPoopers.mutate({ termId, userIds })
  ...
  return (
    <div className="mt-3 border-t pt-3">
          ...
          {current.map((id) => (
            ...
            >
              ...
              <span className="max-w-[10rem] truncate">
                {byId.has(id) ? userLabel(byId.get(id)!) : "Unknown"}
              </span>
              <button
                type="button"
                aria-label={`Remove ${byId.has(id) ? userLabel(byId.get(id)!) : "user"} from Poopers`}
                disabled={setPoopers.isPending}
                onClick={() => save(current.filter((x) => x !== id))}
                className="grid h-6 w-6 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
      ...
      <select
        className={inputClass}
        aria-label={`Add a Zigma Pooper to ${termName}`}
        disabled={setPoopers.isPending}
        value=""
        onChange={(e) => e.target.value && save([...current, e.target.value])}
      >
        <option value="">— Add a Zigma Pooper —</option>
        {users
          .filter((u) => !current.includes(u.id))
          .map((u) => (
            <option key={u.id} value={u.id}>
              {userLabel(u)}
            </option>
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Disqualified: still scored and still on the Podium, greyed out, but never eligible
        to be the Maxxer. They wear a 💩 where the Maxxer wears the crown.
      </p>
    </div>
  )
}
