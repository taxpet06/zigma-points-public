...
import { usePoopHolders } from "@/components/cosmetics/use-poop-holders"
...
export function UserAvatar({
  ...
  const pooped = usePoopHolders(userId)
  ...
  return (
    ...
    <span className="relative inline-flex shrink-0">
      ...
      {/* Never both: a Zigma Pooper cannot be that term's Maxxer (term.setWinner
          refuses them and scoring's winnerOf skips them), so the two badges can
          never contend for this slot. */}
      {!crowned && pooped && <PoopBadge size={size} />}
