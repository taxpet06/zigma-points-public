// End-of-day Sequence Recall leaderboard prizes. The daily-reward-reset cron (00:00 ET)
// calls awardSequenceRecallDailyPrizes(previousDayKey()) to pay the top 3 of the day that
// just closed. Same shape as znake/prizes.ts — ranks by zpEarned, which for Sequence
// Recall is the banked ZP (1 per cleared round).
//
...
/** Credit the top 3 of `day`'s Sequence Recall leaderboard. Safe to call more than once per day. */
export async function awardSequenceRecallDailyPrizes(day: string): Promise<number> {
  ...
  const runs = await db.sequenceRecallRun.findMany({
  ...
  for (const { run, rank, prize } of winners) {
    ...
    if (credited) {
      awarded++
      after(() => notifyLeaderboardPrize(run.userId, "Monkey Test", rank, prize))
    }
  }
