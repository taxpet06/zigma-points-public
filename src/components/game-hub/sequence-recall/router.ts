...
import { claimsCrown } from "@/lib/daily-prizes"
import { stampTermId, leaderboardScopeSchema, leaderboardScopeWhere } from "@/lib/terms"
import {
  FREE_PLAYS_PER_DAY,
  REPLAY_COST,
  ALL_TIME_CROWN_ZP,
 ...
 *  a crown bonus for every board it just took sole #1 on. Takes a transaction client as
 ...
 *  settle from inside its own transaction without nesting one.
 *
 *  Returns the boards whose crowns were paid (empty when none were). Two are claimable
 *  on one run: the all-time board and the run's OWN term's board — passed in as `termId`
 *  from the run row, NOT read fresh, so a run that ends seconds after a new term starts
 *  still settles against the term it was played in. They are separate races and pay
 *  independently; taking both at once is worth 2 x ALL_TIME_CROWN_ZP. A run stamped with
 *  no term has no term board to top, so it is checked against all-time only. */
export async function settleEndedRunInTx(
  tx: Prisma.TransactionClient,
  {
    ...
    zpEarned,
    termId,
  }: { runId: string; userId: string; zpEarned: number; termId: string | null },
): Promise<("all-time" | "term")[]> {
  const { count } = await tx.sequenceRecallRun.updateMany({
    where: { id: runId, status: { not: "ENDED" } },
    data: { status: "ENDED", endedAt: new Date() },
  })
  if (count === 0 || zpEarned <= 0) return [] // lost the race, or unscored — no crown

  const boards: { board: "all-time" | "term"; scope: { termId?: string } }[] = [
    { board: "all-time", scope: {} },
    ...(termId ? [{ board: "term" as const, scope: { termId } }] : []),
  ]
  const crowns: ("all-time" | "term")[] = []
  for (const { board, scope } of boards) {
    const ranked = { status: "ENDED" as const, zpEarned: { gt: 0 }, ...scope }
    const others = await tx.sequenceRecallRun.aggregate({
      _max: { zpEarned: true },
      where: { ...ranked, userId: { not: userId } },
    })
    const mine = await tx.sequenceRecallRun.aggregate({
      _max: { zpEarned: true },
      where: { ...ranked, userId, id: { not: runId } },
    })
    if (!claimsCrown(zpEarned, others._max.zpEarned ?? -1, mine._max.zpEarned ?? -1)) continue
...
export const sequenceRecallRouter = createTRPCRouter({
  ...
  start: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    // Read outside the transaction: it is a tiny indexed lookup that nothing in the
    // tx writes, and holding it inside would widen a Serializable transaction for no
    // gain. A term starting mid-start() is a one-run edge case with no economy impact.
    const termId = await stampTermId()
    ...
    const created = await runSerializable(async (tx) => {
      ...
      const row = await tx.sequenceRecallRun.create({
        data: { userId, day, seed, entryCost, termId, status: "ACTIVE" },
  ...
  submitRound: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const result = await db.$transaction(async (tx) => {
        ...
        const crowns = await settleEndedRunInTx(tx, {
          runId: run.id,
          userId,
      ...
      if (result.runEnded) {
        ...
        for (const board of result.crowns) {
          after(() => notifyLeaderboardPrize(userId, "Monkey Test", 1, ALL_TIME_CROWN_ZP, board))
        }
      }
      const { crowns: _crowns, ...response } = result
      return response
    }),
   ...
   *  submitRound failure branch has already settled a failed run. */
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const run = await db.sequenceRecallRun.findUnique({
        where: { id: input.runId },
        select: { id: true, userId: true, status: true, zpEarned: true, tier: true, round: true, entryCost: true, termId: true },
      })
      if (!run || run.userId !== userId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "Run not found." })
      }
      ...
      const crowns = await db.$transaction((tx) =>
        settleEndedRunInTx(tx, { runId: run.id, userId, zpEarned: run.zpEarned, termId: run.termId }),
      )
      after(() => notifyZpChange(userId))
      // The crown is leaderboard ZP — name the board and the payout, not just "balance changed".
      for (const board of crowns) {
        after(() => notifyLeaderboardPrize(userId, "Monkey Test", 1, ALL_TIME_CROWN_ZP, board))
      }
      return summary
    }),
  ...
  leaderboard: protectedProcedure
    .input(leaderboardScopeSchema)
    .query(async ({ input }) => {
      // null = a term board was asked for but no term exists yet. Falling through to
      // {} would quietly serve the all-time board under a term's name.
      const scopeWhere = await leaderboardScopeWhere(input)
