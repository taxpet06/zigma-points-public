...
import { Coins, Crown, Lock, Medal, RotateCcw, Trophy } from "lucide-react"
...
/** The podium + crown payouts. Rendered in competitive modals and above every
 *  leaderboard, so the prize on offer is visible wherever the ranking is.
 *
 *  `termClosed` swaps the term panel's prize copy for a finality note: a term that
 *  has ended can no longer be climbed, and promising ZP on a board nobody can move
 *  is the one thing this panel must never do. */
export function LeaderboardPrizes({
  scope = "today",
  termClosed = false,
}: {
  scope?: "today" | "term" | "all-time"
  termClosed?: boolean
}) {
  if (scope === "term") {
    if (termClosed) {
      // Deliberately NOT the amber prize panel — nothing is on offer here. Muted
      // surface, no crown, no ZP figure: this is a closed record, not an incentive.
      return (
        <div className="rounded-lg bg-muted/50 px-3 py-2.5 ring-1 ring-border">
          <p className="flex items-center gap-1.5 text-sm font-semibold text-foreground">
            <Lock className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" />
            This term has ended
          </p>
          <p className="mt-1 text-xs text-muted-foreground text-pretty">
            Its leaderboard is final — no new runs can rank on it.
          </p>
        </div>
      )
    }
    return (
      <div className="rounded-lg bg-amber-500/10 px-3 py-2.5 ring-1 ring-amber-500/25">
        <p className="flex items-center gap-1.5 text-sm font-semibold text-foreground">
          ...
          Take sole #1 this term to win{" "}
          <span className="text-amber-600 dark:text-amber-400">+{ALL_TIME_CROWN_ZP} ZP</span>
        </p>
        <p className="mt-1 text-xs text-muted-foreground text-pretty">
          A separate race from all-time — one run can claim both. Paid on paid replays too.
        </p>
      </div>
    )
  }
