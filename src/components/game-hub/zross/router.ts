   ...
   *  the same summary and is a no-op on the DB. */
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const run = await db.zrossRun.findUnique({
        where: { id: input.runId },
        select: { id: true, userId: true, status: true, zpEarned: true, pickupsClaimed: true, entryCost: true, termId: true },
      })
      if (!run || run.userId !== userId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "Run not found." })
      }
      ...
      await db.$transaction(async (tx) => {
        ...
        // Two crowns are claimable on one run: the all-time board and the run's OWN
        // term's board (not the current term — a run that ends seconds after a new term
        // starts still belongs to the term it was played in). They are separate races,
        // so they pay independently; taking both at once is worth 2 x ALL_TIME_CROWN_ZP.
        // A run stamped with no term has no term board to top, so it checks all-time only.
        const boards: { board: "all-time" | "term"; scope: { termId?: string } }[] = [
          { board: "all-time", scope: {} },
          ...(run.termId ? [{ board: "term" as const, scope: { termId: run.termId } }] : []),
        ]
        for (const { board, scope } of boards) {
          ...
          const mine = await tx.zrossRun.aggregate({
            _max: { zpEarned: true },
            where: { ...ranked, userId, id: { not: run.id } },
          })
          if (claimsCrown(run.zpEarned, others._max.zpEarned ?? -1, mine._max.zpEarned ?? -1)) {
            await tx.user.update({
              where: { id: userId },
      ...
      for (const board of crowns) {
        after(() => notifyLeaderboardPrize(userId, "Zross", 1, ALL_TIME_CROWN_ZP, board))
      }
      return summary
    }),
