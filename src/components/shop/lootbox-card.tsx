...
export function LootboxCard({
  ...
  /** Past term: still fully browsable, just not buyable. The card stays a live click
   *  target (looking inside is the point) — only the button's promise changes. */
  readOnly?: boolean
}) {
  ...
  return (
    ...
    >
      ...
      {/* Outline, not primary, on a past term: DESIGN.md reserves the filled brand
          button for the primary action, and here the primary action is gone. */}
      <Button variant={readOnly ? "outline" : "default"} className="min-h-11 w-full">
        {readOnly ? "View contents" : "View"}
      </Button>
    </div>
  )
}
