...
export function LootboxModal({
  ...
  onClose: () => void
  onSuccess: () => void
  /** Past term: the contents, odds and circulation still show — only the open path
   *  is withdrawn. shop.openBox rejects it server-side too, so this is the honest
          ...
          {isBrowsing && (
            <>
              ...
              <div className="space-y-1.5 text-sm text-muted-foreground">
                ...
                <p>
                  Contains one of {items.length} {lootbox.collection} Collection{" "}
              ...
              <div className="flex flex-col gap-2">
                {readOnly ? (
                  // No disabled "Open" button: a greyed-out primary reads as "not yet",
                  // and this is "not ever". State the reason instead, and say what the
                  // user CAN still do with these items, since that part is unchanged.
                  <p
                    role="status"
                    className="rounded-md bg-muted/50 px-3 py-2 text-center text-xs text-muted-foreground text-pretty ring-1 ring-border"
                  >
                    <span className="font-medium text-foreground">
                      {lootbox.collection} has ended.
                    </span>{" "}
                    These boxes can&rsquo;t be opened any more — but their items still
                    turn up on the Listings board and in trades.
                  </p>
                ) : (
