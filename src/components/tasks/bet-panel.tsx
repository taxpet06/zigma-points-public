...
export function BetPanel({ taskId, isAdmin }: { taskId: string; isAdmin: boolean }) {
  ...
  const { choices, minBet, betsCloseAt, locked, termClosed, settled, winningChoice, pot, tally, bettorCount, myBalance, myBet } = data
  ...
  return (
    ...
    >
      ...
      {/* Locked banner — cutoff passed, or the term is over. `locked` already covers
          both (bet.getBetState folds them), so every form gate above closes either way;
          only the reason differs, and "waiting for the result" would be a lie on a pool
          whose term simply ended with nobody settling it. */}
      {locked && !settled && (
        <div className="mt-4 flex items-center gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3.5 py-2.5 text-sm font-medium text-amber-700 dark:text-amber-400">
          ...
          {termClosed
            ? "This pool is from a past term — betting is closed."
