...
export interface TaskCardProps {
  ...
  betSettled?: boolean
  /** Pool is from a past term: betting is closed regardless of cutoff (bet.placeBet). */
  termClosed?: boolean
...
export function TaskCard({
  ...
  //
  // A past term closes a pool even with no cutoff set — the "forgotten pool" case that
  // bet.placeBet refuses. Without it the card advertises "Open · Place your bet" for a
  // pool the server will reject, which is the trap the detail panel already avoids.
  // The term half is pure (a server-computed prop); only the cutoff comparison below
  // reads the clock, so the disable stays pinned to exactly that line.
  const cutoffPassed =
    // eslint-disable-next-line react-hooks/purity
    !betSettled && betsCloseAt != null && betsCloseAt.getTime() <= Date.now()
  const betLocked = termClosed || cutoffPassed
