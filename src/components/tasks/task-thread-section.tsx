...
interface TaskThreadSectionProps {
  ...
  /** Task is from a past term: replies are read-only (see reply.createReply). */
  closed?: boolean
}

export function TaskThreadSection({ taskId, closed = false }: TaskThreadSectionProps) {
  ...
  return (
    <>
      ...
      <TaskReplyThread taskId={taskId} onReply={handleReply} closed={closed} />
    </>
  )
}
