...
export async function signUp(formData: FormData): Promise<SignUpResult> {
  // Server Actions are addressable HTTP endpoints, resolved by the Next-Action header
  // rather than by path, so middleware's redirect does not stop one: it answers a form
  // POST with a method-preserving 307, and the browser re-POSTs the action — header and
  // body intact — at /downtime, where it runs. Without this guard that creates a user row
  // mid-break and then hits the signIn block below, leaving an orphan account behind an
  // AccessDenied page.
  if ((await downtimeState()).down) {
    return { success: false, error: "Zigma Points is closed until the next term begins." }
  }
