...
export async function notifyLeaderboardPrize(
  ...
  rank: number,
  zp: number,
  board: "daily" | "all-time" | "term" = "daily",
): Promise<void> {
  const medal = board === "daily" ? (["🥇", "🥈", "🥉"][rank - 1] ?? "🏆") : "👑"
  const place = ["1st", "2nd", "3rd"][rank - 1] ?? `${rank}th`
  // Term and all-time are both crowns and both pay the same, so the copy has to say
  // WHICH board was taken — otherwise a run that claims both sends two identical pushes.
  const title =
    board === "all-time"
      ? `${medal} ${place} all-time in ${game}`
      : board === "term"
        ? `${medal} ${place} this term in ${game}`
        : `${medal} ${place} place in ${game}`
  const body =
    board === "all-time"
      ? `You took ${place} on the ${game} all-time leaderboard — +${zp} ZP.`
      : board === "term"
        ? `You took ${place} on the ${game} leaderboard for this term — +${zp} ZP.`
