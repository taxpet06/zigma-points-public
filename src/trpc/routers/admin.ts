...
export const adminRouter = createTRPCRouter({
   ...
   */
  /**
   * Open-vs-closed right now, and WHY: the admin override in force, plus what the term
   * schedule alone would say. The panel needs both to explain "closed (forced)" versus
   * "closed (between terms)" and to offer the right buttons.
   */
  downtimeStatus: adminProcedure.query(async () => {
    const state = await downtimeState()
    return {
      down: state.down,
      scheduledDown: state.scheduledDown,
      forcedDown: state.forcedDown,
  ...
  /**
   * Set (or clear) the manual downtime override.
   *   true  — close now, even mid-term.
   *   false — stay open, even between terms.
   *   null  — follow the term schedule again.
   *
   * Upsert on the fixed singleton id, so there is one row and no table to keep tidy.
   *
   * Takes up to lib/downtime.ts's TTL to reach every serverless isolate; each caches
   * the verdict independently and nothing can invalidate them from outside. The button
   * says so rather than pretending the flip is instant.
   */
  setDowntime: adminProcedure
    .input(z.object({ forcedDown: z.boolean().nullable() }))
    .mutation(async ({ input }) => {
      await db.downtimeOverride.upsert({
        where: { id: "singleton" },
        update: { forcedDown: input.forcedDown },
        create: { id: "singleton", forcedDown: input.forcedDown },
      })
      return { forcedDown: input.forcedDown }
    }),
