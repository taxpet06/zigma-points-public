...
export const betRouter = createTRPCRouter({
  ...
  getBetState: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // A pool from a past term takes no more stakes (placeBet enforces it). Folding
      // that into `locked` here rather than threading a prop from the page means every
      // gate the panel already keys off `!locked` closes with it — including the one on
      // a pool with betsCloseAt null, which is exactly the forgotten-pool case. The
      // separate flag is only so the copy can say WHY, since "waiting for the result"
      // is wrong for a term that is simply over.
      const termClosed = !(await isCurrentTerm(task.termId))
      ...
      return {
        ...
        locked:
          termClosed || (task.betsCloseAt !== null && task.betsCloseAt.getTime() <= Date.now()),
        termClosed,
  ...
  placeBet: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const task = await db.task.findUnique({
        where: { id: input.taskId },
        select: { id: true, kind: true, choices: true, minBet: true, betsCloseAt: true, betSettledAt: true, termId: true },
      })
      if (!task || task.kind !== "BET") {
        throw new TRPCError({ code: "NOT_FOUND", message: "Bet not found." })
      }
      // A pool from a past term takes no new stakes. betsCloseAt is nullable ("open
      // until settlement"), so without this a forgotten pool from a closed term keeps
      // accepting ZP forever. Existing stakes are untouched — settleBet and cancelBet
      // stay open to an admin, and cancelling is what refunds them.
      if (!(await isCurrentTerm(task.termId))) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "This pool is from a past term — betting is closed.",
        })
      }
