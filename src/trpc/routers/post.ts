...
export const postRouter = createTRPCRouter({
  ...
  createPost: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Posting needs a term running. Without one the post could never be voted or
      // replied to (isCurrentTerm is false for everything during a break), so it would
      // be an inert husk in the feed — better to say no than to make one.
      ...
      const post = await db.post.create({
        data: {
          authorId,
          // Stamped from the then-current term (lib/terms.ts) so the feed's term
          // filter is a plain indexed equality, not a date range re-derived per read.
  ...
  getFeed: protectedProcedure
    .input(
      z.object({
        ...
        // Term filter. Omitted/null = every term, which is the default the feed
        // ships with: posts are chronological, so "all terms" is already a coherent
        // reading order and a term is a narrowing, not a required choice.
        termId: z.string().nullish(),
      })
    )
    .query(async ({ ctx, input }) => {
      const callerId = ctx.session.user.id
      const { cursor, limit, termId } = input
      // Both halves of the merged feed take the same filter, so a filtered page can't
      // show a term's posts next to another term's betting pools.
      const termWhere = termId ? { termId } : {}
      // Read once for the whole page, not per row: the card needs it to stop showing
      // "Open · Place your bet" on a pool from a term that has moved on.
      const liveTermId = await stampTermId()
      ...
      const [posts, pools] = await Promise.all([
        db.post.findMany({
          where: { type: { in: ["AWARD", "DEDUCT", "REGULAR"] }, ...termWhere, ...keyset },
          take: limit + 1,
        ...
        db.task.findMany({
          where: { kind: "BET", ...termWhere, ...keyset },
          take: limit + 1,
          orderBy: [{ createdAt: "desc" }, { id: "desc" }],
          select: {
            id: true,
      ...
      const mappedPools = pools.map((pool) => ({
        ...
        termClosed: pool.termId !== liveTermId,
        _count: pool._count,
      }))
