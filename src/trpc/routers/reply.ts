...
export const replyRouter = createTRPCRouter({
  ...
  createReply: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // A thread from a past term is READ-ONLY: still fully visible, but closed to new
      // replies (isCurrentTerm in lib/terms). Voting already stops on its own — every
      // post's window is 24h, so an old-term post is long settled — but replies had no
      // such limit at all, which left every archived thread live indefinitely.
      // The termId is selected alongside the existence check, so this costs one extra
      // query (the current term), not two.
      ...
      if (input.postId) {
        ...
        if (!(await isCurrentTerm(post.termId))) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "This post is from a past term — its thread is closed.",
          })
        }
      }
      ...
      if (input.taskId) {
        ...
        if (!(await isCurrentTerm(task.termId))) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "This is from a past term — its thread is closed.",
          })
        }
      }
