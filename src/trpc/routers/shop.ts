...
export const shopRouter = createTRPCRouter({
  ...
  openBox: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Only the CURRENT term's boxes mint. A term's name is its collection key, so
      // this one comparison is the whole rule (see lootboxesForTerm in lib/cosmetics).
      // It lives HERE, not just in the UI: the shop lets you browse a past term's box
      // and its odds, and a client that flipped the browse call into an open call is
      // exactly what would put a retired term's items back into circulation.
      // Note this bounds MINTING only — listing, buying and trading an item from a
      // closed term stay untouched, which is the point: old items keep circulating,
      // no new ones appear.
      const term = await currentTerm()
      if (!term || box.collection !== term.name) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "That lootbox is from a past term and can no longer be opened.",
        })
      }
