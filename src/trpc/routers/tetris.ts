   ...
   *  the one-time ZP credit + crown check so a resend never double-pays. */
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const run = await db.tetrisRun.findUnique({
        ...
        select: { id: true, userId: true, seed: true, startedAt: true, entryCost: true, termId: true },
      })
      ...
      await db.$transaction(async (tx) => {
        ...
        for (const { board, scope } of boards) {
          const ranked = { status: "ENDED" as const, score: { gt: 0 }, ...scope }
          ...
          if (claimsCrown(result.score, others._max.score ?? -1, mine._max.score ?? -1)) {
      ...
      for (const board of crowns) {
        after(() => notifyLeaderboardPrize(userId, "Petris", 1, ALL_TIME_CROWN_ZP, board))
      }
      return summary
    }),
