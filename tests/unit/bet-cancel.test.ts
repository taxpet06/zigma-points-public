...
const dbMock = vi.hoisted(() => ({
  ...
  // createTask both REQUIRES an active term (requireActiveTerm) and stamps the visible
  // one (stampTermId) — both land on db.term.findFirst, so it must return a term or
  // every open is refused with "No term is running right now".
  term: { findFirst: vi.fn(() => ({ id: "term-1", name: "26X", startsAt: new Date(0), endsAt: new Date(8.64e15) })) },
...
describe("task.createTask — who may open what", () => {
  ...
  it("refuses a STANDARD task outright — Activities no longer exist", async () => {
    // This used to be a FORBIDDEN admin gate. With Activities removed, STANDARD is
    // rejected by input validation instead, for everyone including admins — the
    // schema pins kind to the BET literal precisely so an old client fails loudly
    // rather than silently opening a choice-less pool.
    await expect(
    ...
    await expect(
      callTask(session("admin-1", "ADMIN")).createTask({
        title: "Do a thing",
        description: "…",
        kind: "STANDARD",
        zpReward: 5,
