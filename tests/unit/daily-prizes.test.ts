...
import { pickDailyWinners, claimsCrown } from "@/lib/daily-prizes"
...
describe("claimsCrown", () => {
  it("awards when dethroning another user (strictly ahead)", () => {
    ...
    expect(claimsCrown(120, 100, 90)).toBe(true)
  })

  it("awards on an empty board (sentinels -1)", () => {
    expect(claimsCrown(50, -1, -1)).toBe(true)
  })
  ...
  it("no award when already #1 and just beating your own record", () => {
    ...
    expect(claimsCrown(110, 90, 100)).toBe(false)
  })

  it("no award for merely tying the top", () => {
    expect(claimsCrown(100, 100, 50)).toBe(false)
  })

  it("no award for a zero score", () => {
    expect(claimsCrown(0, -1, -1)).toBe(false)
  })
})
