...
describe("daily reward roll", () => {
  it("maps tick count to the ZP tiers (0/0/2/6) and keeps a replay -EV", () => {
    expect(ZP_BY_TICKS).toEqual([0, 0, 2, 6])

    // The tier table is only half the rule — what matters is that spinning COSTS ZP on
    // average. Reels are 3 independent 50/50s, so ticks ~ Binomial(3, 0.5).
    const p = [1 / 8, 3 / 8, 3 / 8, 1 / 8]
    const ev = ZP_BY_TICKS.reduce<number>((sum, zp, ticks) => sum + zp * p[ticks], 0)
    expect(ev).toBeCloseTo(1.5, 10)
    expect(ev).toBeLessThan(SLOTS_REPLAY_COST)
    // Guard the regression this table exists to fix: the old [0,1,3,5] paid E = 2.125
    // against the same 2 ZP replay, which is +EV and let one account run 645,507
    // automated spins in a term. Never let the payout go back above the cost.
  })
