...
const POOLS = [
  ...
  winningChoice: null,
  betSettledAt: null,
  // b0 is the oldest pool and belongs to a term that has since rolled over — it is the
  // "forgotten pool" shape (no cutoff, never settled) the card must not advertise.
  termId: r.id === "b0" ? "term-old" : "term-now",
...
const dbMock = vi.hoisted(() => ({
  post: { findMany: vi.fn() },
  task: { findMany: vi.fn() },
  // getFeed reads the live term once per page to mark past-term pools closed.
...
dbMock.term.findFirst.mockResolvedValue({ id: "term-now", name: "26X", startsAt: T(0), endsAt: T(99) })
...
describe("post.getFeed — merged posts + betting pools", () => {
  it("marks a past-term pool closed so the card can't advertise 'Open'", async () => {
    const page = await caller.getFeed({ limit: 20 })
    const byId = new Map(page.items.map((i) => [i.id, i]))
    expect(byId.get("b0")?.termClosed).toBe(true)  // term rolled over
    expect(byId.get("b8")?.termClosed).toBe(false) // current term
    expect(byId.get("p9")?.termClosed).toBe(false) // posts carry the field too, always false
  })
