...
// The term whose name matches BOX's collection, so openBox's "current term only"
// gate passes by default. Tests that want the closed-term path override it.
const CURRENT_TERM = {
  id: "term-1",
  name: BOX.collection,
...
beforeEach(() => {
  ...
  vi.mocked(dbMock.term.findFirst).mockReset().mockResolvedValue(CURRENT_TERM)
...
describe("shop.openBox", () => {
  ...
  it("refuses a past term's box before any $transaction — no new copies get minted", async () => {
    // The current term is a different collection, so BOX belongs to a closed one.
    vi.mocked(dbMock.term.findFirst).mockResolvedValue({ ...CURRENT_TERM, name: "27X" })
    const caller = createCaller(ctx)

    await expect(caller.openBox({ boxId: BOX.id })).rejects.toMatchObject({ code: "FORBIDDEN" })
    expect(dbMock.$transaction).not.toHaveBeenCalled()
    expect(dbMock.user.updateMany).not.toHaveBeenCalled()
  })

  it("refuses every box when no term has started", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(null)
    await expect(createCaller(ctx).openBox({ boxId: BOX.id })).rejects.toMatchObject({
      code: "FORBIDDEN",
    })
    expect(dbMock.$transaction).not.toHaveBeenCalled()
  })
