...
describe("createTaskSchema", () => {
  ...
  it("defaults kind to BET — the only kind left after Activities were removed", () => {
    const result = createTaskSchema.safeParse(validTask)
    expect(result.success).toBe(true)
    if (result.success) expect(result.data.kind).toBe("BET")
  })

  it("REJECTS kind STANDARD rather than coercing it into a pool", () => {
    // An old client still posting STANDARD must fail loudly. Dropping the field
    // instead would have silently created a choice-less betting pool.
    expect(createTaskSchema.safeParse({ ...validTask, kind: "STANDARD" }).success).toBe(false)
  })

  it("rejects a pool with fewer than 2 distinct choices", () => {
    expect(createTaskSchema.safeParse({ ...validTask, choices: ["A", "A"] }).success).toBe(false)
    expect(createTaskSchema.safeParse({ ...validTask, choices: ["A"] }).success).toBe(false)
  })

  it("rejects a pool with no minimum bet", () => {
    const { minBet: _omitted, ...noMin } = validTask
    expect(createTaskSchema.safeParse(noMin).success).toBe(false)
  })
