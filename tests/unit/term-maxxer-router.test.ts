...
const dbMock = vi.hoisted(() => {
  const mock = {
    ...
    termPooper: { findUnique: vi.fn(), findMany: vi.fn(), deleteMany: vi.fn(), createMany: vi.fn() },
...
beforeEach(() => {
  ...
  // Not a Zigma Pooper unless a test says otherwise.
  dbMock.termPooper.findUnique.mockResolvedValue(null)
})
...
describe("term.setWinner — Zigma Poopers", () => {
  it("refuses a user who is disqualified for that term", async () => {
    dbMock.termPooper.findUnique.mockResolvedValue({ id: "pooper-row" })
    const caller = createCaller(adminCtx)

    await expect(
      caller.setWinner({ termId: "term-1", userId: "user-winner" }),
    ).rejects.toMatchObject({ code: "BAD_REQUEST" })

    // Nothing moved: no winner written, no crown handed over.
    expect(dbMock.$transaction).not.toHaveBeenCalled()
  })

  it("checks the pooper list for THIS term, not globally", async () => {
    const caller = createCaller(adminCtx)
    await caller.setWinner({ termId: "term-1", userId: "user-winner" })
    expect(dbMock.termPooper.findUnique).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { termId_userId: { termId: "term-1", userId: "user-winner" } },
      }),
    )
  })
})
