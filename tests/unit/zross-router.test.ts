...
const dbMock = vi.hoisted(() => {
  const mock = {
    ...
    // currentTermId()/currentTerm() read this on every start() and openBox() —
    // the term stamp and the "current term only" lootbox gate both go through it.
    term: {
      findFirst: vi.fn(),
    },
    user: {
...
describe("zross — terms", () => {
  const TERM = { id: "term-1", name: "26X", startsAt: new Date(), endsAt: new Date() }

  it("start stamps the run with the current term", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(TERM)
    vi.mocked(dbMock.zrossRun.count).mockResolvedValue(0)
    vi.mocked(dbMock.zrossRun.create).mockResolvedValue({ id: "run-1", seed: 7 })
    await createCaller(ctx).start()
    expect(dbMock.zrossRun.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ termId: TERM.id }) }),
    )
  })

  it("start stamps null when no term has begun", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(null)
    vi.mocked(dbMock.zrossRun.count).mockResolvedValue(0)
    vi.mocked(dbMock.zrossRun.create).mockResolvedValue({ id: "run-1", seed: 7 })
    await createCaller(ctx).start()
    expect(dbMock.zrossRun.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ termId: null }) }),
    )
  })

  it("one run can take BOTH crowns, and they stack", async () => {
    vi.mocked(dbMock.zrossRun.findUnique).mockResolvedValue(
      activeRun({ zpEarned: 30, pickupsClaimed: 30, termId: TERM.id }),
    )
    vi.mocked(dbMock.zrossRun.updateMany).mockResolvedValue({ count: 1 })
    vi.mocked(dbMock.zrossRun.aggregate)
      .mockResolvedValueOnce({ _max: { zpEarned: 10 } }) // all-time others
      .mockResolvedValueOnce({ _max: { zpEarned: -1 } }) // all-time mine before
      .mockResolvedValueOnce({ _max: { zpEarned: 8 } }) // term others
      .mockResolvedValueOnce({ _max: { zpEarned: -1 } }) // term mine before
    await createCaller(ctx).end({ runId: "run-1" })
    // Two separate credits of ALL_TIME_CROWN_ZP, not one.
    expect(dbMock.user.update).toHaveBeenCalledTimes(2)
    expect(notifyLeaderboardPrize).toHaveBeenCalledWith(USER_ID, "Zross", 1, ALL_TIME_CROWN_ZP, "all-time")
    expect(notifyLeaderboardPrize).toHaveBeenCalledWith(USER_ID, "Zross", 1, ALL_TIME_CROWN_ZP, "term")
  })

  it("takes the term crown alone when someone else still holds all-time", async () => {
    vi.mocked(dbMock.zrossRun.findUnique).mockResolvedValue(
      activeRun({ zpEarned: 30, pickupsClaimed: 30, termId: TERM.id }),
    )
    vi.mocked(dbMock.zrossRun.updateMany).mockResolvedValue({ count: 1 })
    vi.mocked(dbMock.zrossRun.aggregate)
      .mockResolvedValueOnce({ _max: { zpEarned: 500 } }) // all-time others — far ahead
      .mockResolvedValueOnce({ _max: { zpEarned: -1 } })
      .mockResolvedValueOnce({ _max: { zpEarned: 8 } }) // term others
      .mockResolvedValueOnce({ _max: { zpEarned: -1 } })
    await createCaller(ctx).end({ runId: "run-1" })
    expect(dbMock.user.update).toHaveBeenCalledTimes(1)
    expect(notifyLeaderboardPrize).toHaveBeenCalledTimes(1)
    expect(notifyLeaderboardPrize).toHaveBeenCalledWith(USER_ID, "Zross", 1, ALL_TIME_CROWN_ZP, "term")
  })

  it("a run with no term stamp is only ever checked against all-time", async () => {
    vi.mocked(dbMock.zrossRun.findUnique).mockResolvedValue(
      activeRun({ zpEarned: 30, pickupsClaimed: 30, termId: null }),
    )
    vi.mocked(dbMock.zrossRun.updateMany).mockResolvedValue({ count: 1 })
    vi.mocked(dbMock.zrossRun.aggregate)
      .mockResolvedValueOnce({ _max: { zpEarned: 10 } })
      .mockResolvedValueOnce({ _max: { zpEarned: -1 } })
    await createCaller(ctx).end({ runId: "run-1" })
    expect(dbMock.zrossRun.aggregate).toHaveBeenCalledTimes(2) // no term pair
    expect(dbMock.user.update).toHaveBeenCalledTimes(1)
  })

  it("the term board filters by termId, and an explicit term beats the current one", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(TERM)
    vi.mocked(dbMock.zrossRun.groupBy).mockResolvedValue([])
    await createCaller(ctx).leaderboard({ scope: "term", termId: "term-OLD" })
    expect(dbMock.zrossRun.groupBy).toHaveBeenCalledWith(
      expect.objectContaining({ where: { status: "ENDED", termId: "term-OLD" } }),
    )
  })

  it("the term board defaults to the current term when none is named", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(TERM)
    vi.mocked(dbMock.zrossRun.groupBy).mockResolvedValue([])
    await createCaller(ctx).leaderboard({ scope: "term" })
    expect(dbMock.zrossRun.groupBy).toHaveBeenCalledWith(
      expect.objectContaining({ where: { status: "ENDED", termId: TERM.id } }),
    )
  })

  it("a term board with no term at all is empty — never the all-time board", async () => {
    vi.mocked(dbMock.term.findFirst).mockResolvedValue(null)
    const rows = await createCaller(ctx).leaderboard({ scope: "term" })
    expect(rows).toEqual([])
    expect(dbMock.zrossRun.groupBy).not.toHaveBeenCalled()
  })
})
